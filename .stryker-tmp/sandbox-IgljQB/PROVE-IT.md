...
starreckon's pitch is "nothing leaves your machine." You should not take that on
...
package is **`starreckon`** (the bare name `starreckon` belongs to an
unrelated 2017 package — `npx starreckon` is not this tool). Both the package and
the repo are live: `npm view starreckon` and
`github.com/Alexander-Sorrell-IT/starreckon`. Every `starreckon …` command
...
```
sh bin/starreckon-proof.sh          # macOS; plain POSIX shell, short enough to read first
```
...
```
node src/cli.mjs prove             # or: starreckon prove
```
...
JS layer. The kernel sits below all of them. Run starreckon inside an OS
sandbox that denies network syscalls and the question is closed for the whole
process tree, no trust in starreckon required.
...
## 2. `starreckon verify` — what each check covers, and what it doesn't

```
node src/verify.mjs        # from a checkout
node src/cli.mjs verify    # same checks, through the CLI
starreckon verify       # the published package
```
...
Independent of everything starreckon says about itself:
...
Every run writes `~/.starreckon/audit/run-<timestamp>.json` (schema v2):
...
the per-month star SVGs under `~/.starreckon/stars` —
...
chain. `starreckon verify` re-walks the chain.
...
`STARRECKON_CONFINEMENT` on the sandboxed child so the log can record which
sandbox the run claims to have been launched under: `bin/starreckon-proof.sh`,
and the command `prove` prints for you to run (it is built by
`buildProofCommand()`, which launches the child through `/usr/bin/env
STARRECKON_CONFINEMENT=<mode>` — that is why the printed string and the executed
...
the **weaker** form and says so on screen: starreckon is running a check on
starreckon, and a build that wanted to lie could lie there. The strong form is
unchanged and is one line away — `sh bin/starreckon-proof.sh`, run by you, in
...
`run_index` is mirrored in `~/.starreckon/audit-counter.json`, which lives
...
```
node src/cli.mjs --reset-audit="why I did this"   # or: starreckon --reset-audit
```

It deletes every run log in `~/.starreckon/audit` and starts a new chain whose
...
`~/.starreckon/audit-counter.json` along with them — otherwise `verify` keeps
...
`npx starreckon` will execute a tarball from the npm registry. A grep of
this repo proves **this repo** — it proves nothing about that tarball unless
you check they match.

**First, the name.** The package is `starreckon`. `npm pack starreckon` (no
...
# expect the whole source tree under package/src/ plus package/bin/starreckon-proof.sh
...
```
npm pack starreckon@<version> --silent          # fetches the EXACT published artifact
npm view starreckon@<version> dist.shasum dist.integrity

shasum -a 1 starreckon-<version>.tgz            # must equal dist.shasum
openssl dgst -sha512 -binary starreckon-<version>.tgz | base64
...
```
tar -xzf starreckon-<version>.tgz               # unpacks into ./package
...
work against the published package (`starreckon@0.6.1`). This variant needs
...
shasum -a 1 /tmp/sfpack/starreckon-0.6.1.tgz                     # equals the printed shasum (SHA-1)
openssl dgst -sha512 -binary /tmp/sfpack/starreckon-0.6.1.tgz | base64
                                                   # equals the printed integrity, minus "sha512-"
tar -xzf /tmp/sfpack/starreckon-0.6.1.tgz -C /tmp/sfpack
...
What starreckon writes, exhaustively:

- `~/.starreckon/` — reports, snapshots, per-month star SVGs (`stars/`), audit
...
- `--join-fleet=DIR` — a directory **you** name. If you name a synced or
  ...
  applies if you sync `~/.starreckon/snapshots` between machines, which the
  README suggests: that is deliberate egress under your control.

So the honest statement is: starreckon's outputs stay local **until you place
...
which you can read yourself: `~/.starreckon` holds plain JSON/SVG/HTML, nothing
