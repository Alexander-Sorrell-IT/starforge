...
test("a default run writes ~/Desktop/starreckon/<date>/ — a write the banner does not disclose", () => {
  ...
  const [folder] = readdirSync(base);
  assert.ok(folder, "the Desktop snapshot folder is empty");
  for (const f of ["report.txt", "star.svg"])
    assert.ok(existsSync(join(base, folder, f)), `Desktop snapshot is missing ${f}`);
  assert.ok(readFileSync(join(base, folder, "star.svg"), "utf8").includes("<svg"), "star.svg is not an SVG");
